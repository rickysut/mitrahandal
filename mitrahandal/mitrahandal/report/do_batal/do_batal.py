...
log_file = os.path.join(log_dir, "do_batal_debug.log")
...
def get_columns():
    return [
		...
		{"label": "Delivery Note", "fieldname": "delivery_note", "width": 260},
        {"label": "Reason", "fieldname": "reason", "width": 280},
...
def get_data(filters):
    ...
    for stop in stops:
        if stop.delivery_note:
            ...
            dn_map[stop.delivery_note]["customer"] = stop.customer
    ...
    for delivery_note, info in dn_map.items():
        ...
        if len(info["stops"]) >= 2:
            ...
            # Filter additional filters from UI
            if filters.get("customer") and info["customer"] != filters.get("customer"):
                log_debug(f"  - Filtered out by customer: {info['customer']} != {filters.get('customer')}")
                continue

            if filters.get("warehouse") and info["warehouse"] != filters.get("warehouse"):
                log_debug(f"  - Filtered out by warehouse: {info['warehouse']} != {filters.get('warehouse')}")
                continue
            ...
            # Only add warehouse if it's not null
            if info["warehouse"]:
                row_data["warehouse"] = info["warehouse"]
